...
export async function fetchSitemap() {
    ...
    for (const entry of list) {
        if (!entry?.loc)
            continue;
        const url = entry.loc;
        if (!url.startsWith(QMK_BASE))
            continue;
        urls.push({ url, lastmod: entry.lastmod });
    }
    return urls;
}
async function listFromSidebar() {
...
export async function fetchDoc(urlOrPath) {
...
export async function listDocs(limit = 100) {
    try {
